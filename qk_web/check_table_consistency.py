...
def get_table_fields(table_name):
    """
    函数 get_table_fields 检索 Hive 数据库中指定表的字段名称和类型。
    
    :param table_name: “table_name”参数是一个字符串，表示要检索其字段名称和类型的表的名称。
    :return: 函数“get_table_fields”返回两个列表：“fields”和“types”。这些列表包含指定表中列的字段名称和数据类型。
    """
...
if __name__ == "__main__":
    ...
    # Hive表名
    hive_table = 'deye_dw_dwd.dwd_voip_store'
    
    run(source_table, hive_table)